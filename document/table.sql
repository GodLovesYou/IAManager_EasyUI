...
CREATE TABLE `IA_FaceDBInfo`(
    ...
    `Size` bigint(8) NULL DEFAULT 10000 COMMENT '库最大容量',
    `Count` bigint(8) NULL DEFAULT 0 COMMENT '库内图片的数量',
...
CREATE TABLE `IA_ManInfo` (
  `Index` bigint(8) unsigned NOT NULL AUTO_INCREMENT COMMENT '索引',
  `Number` varchar(32) NOT NULL COMMENT '编号（针对外委队伍）',
  `JobNumber` varchar(32) DEFAULT NULL COMMENT '工号（针对供电公司员工）',
  ...
  `SubcontractTeam` varchar(255) DEFAULT NULL COMMENT '所属分包队伍（针对外委队伍）',
  `Dept` varchar(255) DEFAULT NULL COMMENT '部门（针对供电公司员工）',
  `Company` varchar(255) DEFAULT NULL COMMENT '公司（针对供电公司员工）',
  `BirthDate` date DEFAULT NULL COMMENT '出生年月',
  `IDNumber` varchar(32) NOT NULL COMMENT '身份证号',
  `PersonalFile_Index` bigint(8) DEFAULT NULL COMMENT '个人档案（图片）索引',
  `ID_A_Index` bigint(8) DEFAULT NULL COMMENT '身份证正面（图片）索引',
  `ID_B_Index` bigint(8) DEFAULT NULL COMMENT '身份证背面（图片）索引',
  `PaperPhoto_Index` bigint(8) DEFAULT NULL COMMENT '试卷照片（图片）索引',
  `ExamResult` int(8) DEFAULT NULL COMMENT '考试成绩',
  `MedicalReport_Index` bigint(8) DEFAULT NULL COMMENT '体检报告（图片）索引',
  `SpecialWorkPermit_Index` bigint(8) DEFAULT NULL COMMENT '特种作业证（图片）索引',
  `AuditStatus` int(8) DEFAULT NULL COMMENT '审核状态',
  `AuditBeginTime` date DEFAULT NULL COMMENT '审核开始有效期',
  `AuditEndTime` date DEFAULT NULL COMMENT '审核结束有效期',
  `Auditor` varchar(255) DEFAULT NULL COMMENT '审核人',
  `AuditTime` datetime DEFAULT NULL COMMENT '审核时间',
  `IllegalInformation` varchar(255) DEFAULT NULL COMMENT '违章信息',
...
CREATE TABLE `IA_GroupInfo` (
  `Index` bigint(8) NOT NULL AUTO_INCREMENT COMMENT '索引',
  `Group` varchar(255) NOT NULL COMMENT '组别名称',
  `Parent_Index` bigint(8) NOT NULL DEFAULT '0' COMMENT '父级索引',
...
CREATE TABLE `IA_ManGroupMap` (
  `Index` bigint(8) NOT NULL AUTO_INCREMENT COMMENT '索引',
  `GroupInfoIndex` bigint(8) NOT NULL COMMENT '组别信息表索引',
  `ManInfoIndex` bigint(8) NOT NULL COMMENT '人员信息表索引',
  `Remark` text NULL COMMENT '备注',
  PRIMARY KEY (`Index`),
  UNIQUE KEY `index` (`GroupInfoIndex`,`ManInfoIndex`) USING BTREE
...
CREATE TABLE `IA_ManFeatureInfo` (
  `Index` bigint(8) NOT NULL AUTO_INCREMENT COMMENT '索引',
  `ManInfo_Index` bigint(8) NOT NULL COMMENT '人员信息表索引',
  `FaceDBInfo_Index` bigint(8) NOT NULL COMMENT '人脸数据库表索引',
